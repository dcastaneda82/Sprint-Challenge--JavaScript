...
graduates.forEach((student) => {
  if (student.university.match('Uni')) {
...
zooAnimals.forEach((animal) => {
  function info() {
    return `${animal.animal_name} ${animal.scientific_name}`;
  }
  animalNames.push(info());
});
...
const lowerCase = [];
const mini = zooAnimals.map(name => {
...
const low = zooAnimals.filter(animal => {
  if (animal.population <= 5) {
    return `${animal.name}`;
  }

});
lowerPopulation.push(low);
...
const take = (totaling, currentTotal) => totaling + currentTotal;
let population = [];
zooAnimals.forEach(item => {
      population.push(item.population);
    });
    populationTotal = population.reduce(take);
